...
from src.chat_llms.base import ChatCompletionChunk

def test_openai_arun():
    import asyncio

    async def arun():
        from src.chat_llms.openai import ChatOpenaiLLM
...
def test_openai_structured_extraction():
    ...
    async def astructured_extraction():
        ...
        llm.add_message({"role": "user", "content": "What is the capital of France?"})
        llm.add_message({"role": "assistant", "content": "The capital of France is Paris."})
        llm.add_message({"role": "user", "content": "What is the capital of Germany?"})
        llm.add_message({"role": "assistant", "content": "The capital of Germany is Berlin."})

        class ExtractedData(BaseModel):
            questions: Annotated[list[str], Field(description="The questions asked by the user")]
